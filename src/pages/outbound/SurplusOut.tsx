...
export default function SurplusOut() {
  ...
  const onSkuChange = (code: string) => {
    ...
    setLocationOptions(items.map((i) => ({ label: `${i.location}（库存: ${i.quantity}）`, value: i.location })));
    ...
    modal.confirm({
      ...
      onOk: async () => {
        try {
          ...
          setSurplus(data); setSelectedItem(null); setLocationOptions([]);
  ...
  return (
    <Form form={form} layout="vertical" onFinish={onFinish} style={{ maxWidth: 480 }} initialValues={{ date: dayjs() }}>
      ...
      <Form.Item label="库位" name="location" rules={[{ required: true, message: '请选择库位' }]}>
        <Select placeholder={locationOptions.length ? '请选择库位' : '请先选择 SKU'}
      ...
      <Form.Item label="出库数量" name="quantity" rules={[
        ...
        <InputNumber ref={qtyRef} min={1} max={selectedItem?.quantity ?? undefined}
          placeholder="请输入数量" style={{ width: '100%' }} onPressEnter={() => form.submit()} />
      </Form.Item>
